...
本计划为 **G-06-01-D** 的补测（`EnemyContext.deleteAura` 是 D-27/D-30 要求「每个公开方法
至少一条正常用例」的公开接口方法，`06-01-PLAN.md` 的 must_haves 逐字点名，但此前全仓库
0 测试引用，同清单的 `addAura` 已覆盖 8 处）；本计划**不引入新码**（`addAura`/`deleteAura`
无对应 warn/error code，不新增也不改动任何码断言）；阶段结构为 构件（单方法正常用例）→
收口（覆盖表小节 + 门禁复核）；只改测试与覆盖表，不改动任何生产/核心源码。

| 缺口 | 用例 | 文件 | 计划 |
| --- | --- | --- | --- |
| G-06-01-D | `applies a global aura after addAura and stops applying it after deleteAura`（`it.skip`，受 `#06-15-1` 阻塞） | `data-system/src/combat/context.test.ts` | 06-15 |

该用例按 D-05 以**正确预期**编写并标记 `it.skip`：`addAura` 生效的前两条断言通过
（目标怪 `atk` 由基础 `2` 变为 `5`），`deleteAura` 传入**同一实例**并再次 `buildup()` 后的
回退断言失败（实际仍为 `5`，期望回到 `2`）。根因与既有 `#06-01-4` **相同**
（`buildup()` 未在重建前对各视图 `reset()`，删除光环后此前已施加的加成不被撤销），
已在 `06-TEST-FINDINGS.md` 登记为 `#06-15-1` 并交叉引用 `#06-01-4`；
修复 `#06-01-4` 后本用例可直接取消 skip。

本计划**无新增码**；不削弱也不删除任何既有用例或跳过（`context.test.ts` 既有用例与断言零变化，
既有 `it.skip` 数量不变）；新增 1 条受阻塞的正确预期 `it.skip`（`#06-15-1`）；
`pnpm test:ci` 全绿。
