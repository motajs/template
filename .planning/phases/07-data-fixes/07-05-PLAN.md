...
<objective>
修复地图数据模型（`@user/data-base` L1）的 2 条登记缺陷：`#06-09-3` `DynamicTile.loadState` 不恢复 `num`（中）、`#06-06-1` 越图 `transferToDynamic` 发错诊断码 131（应为 128，对齐两个 `transferToStatic*` 兄弟分支，D-04）。
...
<task type="auto">
  <name>Task 1: 修复 #06-09-3 —— DynamicTile.loadState 恢复图块数字</name>
...
<task type="auto">
  <name>Task 2: 修复 #06-06-1 —— 越图 transferToDynamic 改发码 128</name>
...
|-----------|----------|-----------|----------|-------------|-----------------|
| T-7-03 | Tampering | `mapLayer.ts` `transferToDynamic` | low | mitigate | Task 2 使越界转换发码 128（越界语义），恢复诊断码的可解释性与唯一性 |
| T-7-11 | Tampering | `dynamicTile.ts` `loadState` | medium | mitigate | Task 1 补齐 `num` 恢复，避免读档后图块数字与存档不一致（静默数据损失） |
