...
<task type="checkpoint:decision" gate="blocking-human">
  ...
  <record>
    ...
    - **并行改动纪律（用户 2026-09-17 指示）**：执行期间用户会并行修改仓库其它文件，**可能产生与本计划无关的类型错误**；执行者**只对本计划声明的路径负责**——文件级 `vue-tsc` 门禁按改动文件相对路径过滤、忽略其它路径的诊断；仅 stage/commit 本计划声明的文件，绝不 `git add -A` / `git commit -a` / `git stash`。
  </record>
