...
must_haves:
  truths:
    ...
    - "四条缺陷各一次原子提交（D-13），提交一律按显式路径 `git add <file>`；本计划**声明路径**（`files_modified` 的 6 个文件 + 本计划的 `07-15-SUMMARY.md`）之外的任何工作树改动（含既有未暂存修改/删除、未跟踪文件，以及 `.planning/ROADMAP.md`/`.planning/STATE.md`——后者的跟踪同步为 orchestrator 收尾步骤）一律不触碰、不暂存、不提交"
  artifacts:
    - path: packages-user/data-base/src/hero/equipment.ts
      provides: "`compareEquip`（`:255-331`）按 Q1=A 改为「克隆加入 / 按同一批克隆删除」：新增局部数组收集 `[name, copy]`（元素类型取自 `stateA.getModifiers()` 的自身元素类型，故零新增 import、不出现 `IHeroModifier` 类型名），加入端 `const copy = modifier.clone()` 后 `clone.addModifier(name, copy)`，删除端遍历该数组 `clone.deleteModifier(name, copy)`；现役槽位删除块（`:274-290`）与方法返回值、146 早退分支、`:319` 注释零改动"
  ...
  prohibitions:
    ...
    - "不得暂存或提交本计划**声明路径**之外的任何路径（声明路径 = `files_modified` 的 6 个文件 + 本计划的 `07-15-SUMMARY.md`；`.planning/ROADMAP.md` 与 `.planning/STATE.md` 由 orchestrator 在本计划收尾后同步，本计划零编辑/零暂存/零提交）；工作树中已存在的无关未暂存修改/删除与未跟踪文件同样不得被暂存或提交：提交一律 `git add <显式路径>`；禁止 `git add -A`/`git add -a`、`git stash`、`git clean`、`git reset --hard`"
...
<task type="checkpoint:decision" gate="blocking-human">
  ...
  <record>
    ...
    - **Q1 = A**：CR-01 在比较克隆上**逐个 `modifier.clone()` 后 `addModifier`**，采样后删除**同一批克隆对象**；活属性上的原修饰器保持绑定、零扰动。对外可观测后果：现役装备参与比较时差值回到契约要求的符号与大小（实算对照：`-7`；修复前 `-12`）；不改变返回类型与两个 146 早退分支。
    - **Q2 = A**：WR-01 的 `normalizeParam` 回退分支改为 `return null`，与该方法自身 jsDoc 的「无法用现有编码表示时返回 `null`」及 `normalizeParamList` 的「无法表示的参数会被丢弃」一致；告警 148 仍发出。对外可观测后果：含非法参数的命令其**参数计数**由 1 变 0（`get(i).params` 的个数随之变化），后续命令的字节偏移不再错位；不新增占位、不改既有 jsDoc。
    - **Q3 = A**：WR-02 的构造器校验由 `< 1` 收紧为 `<= 1`（告警 149 + 回退 2），与告警 149 文案「必须大于 1」一致；命令与参数两个分支同改。对外可观测后果：被接受的配置范围收紧——传 1 从「静默接受（随后栈溢出）」变为「告警 149 并按 2 处理」；已核查仓库内无配置/用例传 1（默认 1.2、测试用 2）。
    - **Q4 = A**：WR-03 的 `clone()` 恢复与 `addModifier` 同等的完整簿记——`modifierName`、`bindAttribute(cloned)`、按源修饰器可存性镜像 `modifierNosave`、再 `recalculateAttribute(name)`；不改 `cloneModifier: false` 的既有语义。对外可观测后果：克隆属性的 `saveState().modifiers` 不再为空、`iterateModifiers()` 可枚举、`getModifierIndex()` 可定位、克隆修饰器 `setValue` 能通知克隆重算。
    - **命名预审（AGENTS.md 要求）**：用户已确认——本计划**不新增任何公共/受保护/私有方法或字段**（生产代码零新增符号）；仅新增方法内局部变量 `copy` / `addedA` / `modifiersA` 及 5 条新用例标题。
    - **未确认观察（不登记、不纳入）**：Task 0 context 中标注「未确认、不登记」的其它退化扩容配置（乘数恰为 1 之外者）**不**作为缺陷登记，也**不**在 Q3 一并封口。
    - **并行改动纪律（用户 2026-09-17 指示）**：执行期间用户会并行修改仓库其它文件，**可能产生与本计划无关的类型错误，也可能导致与本计划无关的测试失效**；执行者**只对本计划声明的路径负责**——文件级 `vue-tsc` 门禁按改动文件相对路径过滤、忽略其它路径的诊断；`pnpm test:ci` 若出现失败，只对**本计划 6 个文件对应的测试**与**由本计划改动引入的失败**负责（沿用执行前重录的基线做对照），其它文件/用例的失败归因于用户并行改动、**不必理会**，不得因此改动计划外代码或放宽本计划断言；仅 stage/commit 本计划声明的文件，绝不 `git add -A` / `git commit -a` / `git stash`。
  </record>
  <resume-signal>已裁决（2026-09-17）：Q1=A, Q2=A, Q3=A, Q4=A（各分支的对外可观测后果见 `<record>`）；命名预审已确认；并行改动纪律已写入。执行者核对 `<record>` 后直接进入 Task 1；若记录与用户实际意图不符，暂停并请用户澄清后再继续</resume-signal>
  <action>向用户汇报四处口径，逐条讲清取舍与**对外可观测后果**：Q1（CR-01）A 分支在比较克隆上克隆加入/克隆删除、活属性零扰动，B 分支会制造「修饰器暂时未绑定」的窗口且本仓库禁用 try-catch 无法兜底，C 分支等于把一个会算错差值的公开 API 留成调用约定，并给出实算对照（现役剑场景下 `-12` vs 契约要求的 `-7`）；Q2（WR-01）A 分支丢弃该参数（命令参数计数、`get(i).params` 个数与后续字节偏移同时变化，与 bigint 溢出路径同口径），B 分支保留 2 字节占位并必须改写两处既有 jsDoc/注释；Q3（WR-02）A 分支把乘数 1 从「静默接受（随后栈溢出）」改为「告警 149 并回退为 2」（收紧被接受的配置范围，已核查无既有配置/用例受影响），B 分支保留乘数 1 但改为严格增大并顺带封口其它退化配置，代价是码 149 文案与实现永久不一致；Q4（WR-03）A 分支恢复四处簿记，后果是**克隆属性 `saveState()` 的 `modifiers` 不再为空**、克隆修饰器可遍历可定位且 `setValue` 能通知克隆重算，B 分支不继承存档开关（源上不入档的修饰器在克隆上会变回可存档），C 分支会破坏 `cloneModifier` 的既有语义与既有用例。同时转述 context 中标注为「未确认、不登记」的观察（乘数恰为 1 之外的退化配置），请用户决定是否在 Q3 选 B 一并封口。**不得在裁决前修改任何文件**；不得自行采纳任何建议默认值。另按 AGENTS.md 的命名要求一并提请批准：本计划**不新增任何公共/受保护/私有方法或字段**（生产代码零新增符号），只新增三个方法内局部变量名 `copy`（被比较修饰器的克隆副本）、`addedA`（A 的克隆收集数组）与 `modifiersA`（`[...stateA.getModifiers()]` 的展开副本，用于让 `addedA` 取得与源迭代器同型的元素类型，见 Task 1），以及 5 条新用例的英文标题（见 objective 的「Artifacts this phase produces」）；若用户对任一命名有异议，先修订本计划再执行。</action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - 命名已按 AGENTS.md 提请批准：确认本计划不新增任何方法/字段，且局部变量名 `copy`/`addedA`/`modifiersA` 与 5 条新用例标题已获用户同意（或已按用户意见改写本计划）
  </acceptance_criteria>
...
<task type="auto">
  ...
  <read_first>
    ...
    - packages-user/data-base/src/hero/types.ts（:671-674 `IEquipmentState.getModifiers(): Iterable<[SelectKey<THero, number>, IHeroModifier<number>]>` 与 :710-714 `IHeroEquipsStore.get(uid): IEquipmentState<THero> | null`——`stateA.getModifiers()` 的静态元素类型即新增收集数组的元素类型来源；**`equipment.ts:3-10` 的 `./types` 具名引入不含 `IHeroModifier`**，该类型在本文件不在作用域内，而本计划禁止新增引入，故收集数组一律以「源迭代器自身类型」标注、不写类型名）
  ...
  <action>
    ...
    (1) **A 的加入端（`:297-300`）**：在循环前新增两行——`const modifiersA = [...stateA.getModifiers()];` 与 `const addedA: typeof modifiersA = [];`（收集数组的元素类型**取自源迭代器自身**，即 `IEquipmentState<THero>.getModifiers()` 的 `Iterable<[SelectKey<THero, number>, IHeroModifier<number>]>`，见 `hero/types.ts:671-674`）。**不得**把元素类型写成显式元组标注：`IHeroModifier` 在本文件**不在作用域内**（`equipment.ts:3-10` 的 `./types` 具名引入只绑定 `EquipStatus`/`IEquipmentState`/`IHeroAttribute`/`IHeroEquipment`/`IHeroEquipmentSave`/`IHeroEquipsStore`；只有 `SelectKey`/`SelectType` 是 `src/types/declaration/util.d.ts:917-924` 的全局环境类型），而本计划（A2 与 prohibitions）禁止新增引入，故**不写类型名、不新增 import、不用 `any`/`as`/`as const`**。循环体改为三行并遍历 `modifiersA`：`const copy = modifier.clone();` → 保留原有那一行 `// @ts-expect-error 泛型无法推导` 注释**逐字不动**（`copy` 的静态类型与今天的 `modifier` 完全相同——`clone()` 的签名是 `clone(): IHeroModifier<H, V>`、`H`/`V` 均取自 `modifier` 自身类型，该注释仍对应同一处类型错误） → `clone.addModifier(name, copy);` → `addedA.push([name, copy]);`。原 `modifier` 变量仍用于 `clone()`，不得被加入比较克隆。
    (2) **A 的删除端（`:305-308`）**：把「再次遍历 `stateA.getModifiers()` 并 `clone.deleteModifier(name, modifier)`」改为「遍历 `addedA` 并 `clone.deleteModifier(name, copy)`」；同样保留该处原有的 `// @ts-expect-error 泛型无法推导` 注释逐字不动（`copy` 与今天的 `modifier` 静态类型相同，故该注释仍是必要的）。删除目标必须是**上一步加入的那个克隆对象**（`deleteModifier` 以 `indexOf` 定位，同一对象即可命中）。
  ...
  <verify>
    ...
    <fails_when>现役装备参与比较时 `compareEquip(sword, axe, 0).atk` 不等于 -7（或对称方向不等于 7）；或比较过程仍产生告警 108；或活属性上原修饰器的索引变为负 / 活属性 final 数值被改变；或 `pnpm exec vue-tsc --noEmit` 输出含本文件路径（收集数组的元素类型必须由 `stateA.getModifiers()` 推导，不得出现作用域外的类型名或新增 import）；或既有 `diffs the final attributes of two equipment instances`、`keeps foreign modifiers when the equipped instance was rebuilt`、两条码 146 用例与 `state.test.ts` 出现回归 —— 按 D-09 退出并修订计划，不得改用「临时解绑活属性修饰器」「比较前先卸下现役装备」「只写文档」等未选分支</fails_when>
  </verify>
  <acceptance_criteria>
    ...
    - 收集数组的元素类型取自 `stateA.getModifiers()`（`const addedA: typeof modifiersA = []`），**未新增任何 import**、未出现类型名 `IHeroModifier`、未使用 `any`/`as`/`as const`/类型别名或接口；`vue-tsc --noEmit` 对本文件 0 命中（Task 5 门槛 (b) 必须在此前即为真，不得推迟到 Task 5 才暴露）
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/ROADMAP.md（Phase 7 段的 Plans 列表与 Wave 分组——**只读参照**：确认 Phase 7 的既有计划行与 Wave 段的现有形态，供 SUMMARY 写明「跟踪同步待 orchestrator 定点追加」的对照；本计划**不编辑**该文件）
  ...
  <action>
    ...
    (4) **范围隔离核对（必做）**：本计划的**声明路径集合** = `files_modified` 的 6 个源/测试文件 + 本计划的产出物 `.planning/phases/07-data-fixes/07-15-SUMMARY.md`（按本阶段 07-12/07-13 先例，frontmatter `files_modified` 只列源/测试文件，SUMMARY 由本 Task 的 `<files>` 声明）。用显式路径区间对比本计划起点 sha..HEAD——`git diff <BASE_SHA>..HEAD --stat -- <上述 7 条路径>`（`BASE_SHA` = 本计划开始执行时的 HEAD 显式 sha），并另跑一次不带路径的 `git diff <BASE_SHA>..HEAD --stat`，断言**区间内出现的文件全部落在这 7 条声明路径内**：Phase 7 的跟踪同步（`.planning/ROADMAP.md`、`.planning/STATE.md`）由 orchestrator 在本计划收尾后**单独提交**（见 (7)），不在本计划的提交区间内；工作树中既有的无关未暂存修改/删除与未跟踪文件不得被暂存或提交。若区间内出现第 8 条路径，按 D-09 退出汇报。同时核对：未新建任何源码/测试文件、未改 `package.json`/`pnpm-lock.yaml`、未改 `hero/types.ts` 与 `replay/types.ts`、未改 `replay/system.ts`/`flag/**`/`combat/mapDamage.ts`、未新增/修改告警码；`packages/common/src/logger.json` 在 `git diff BASE_SHA..HEAD -- packages/common/src/logger.json` 下为空（即本计划未改动它——该文件已由用户自行修改，**不得**以工作树存在改动判定越界）。
    ...
    (7) **Phase 7 验证失效与跟踪同步（orchestrator 负责，本计划不落笔）**：SUMMARY 明写 `07-VERIFICATION.md`（`_Verified: 2026-09-17T10:42:49Z_`）已被本批改动与更早的 07-09..07-14 失效，本计划执行完毕后**必须重跑 `/gsd-verify-work`（或 `gsd-verify-work 7`）**重新出具验证结论。`.planning/ROADMAP.md` 的 Phase 7 段（Plans 计数 + `07-15-PLAN.md` 计划行 + Wave 15 段）与 `.planning/STATE.md` 的同步是**本计划之外的 orchestrator 收尾步骤**——本阶段先例：07-10/07-11 的 SUMMARY 均明写「`ROADMAP.md` / `STATE.md` 未由本计划更新（orchestrator owns those）」，git 记录中该同步是独立的 `docs(phase-07): update tracking after plan …` 提交，07-12 的计划亦明写「本计划**不改** `ROADMAP.md` 文本」。故本计划**不编辑、不暂存、不提交** `.planning/ROADMAP.md` 与 `.planning/STATE.md`；SUMMARY 只承担以下声明：跟踪同步待 orchestrator 以**定点追加**方式完成（既有计划行与既有说明段不得被重排/删除/改写），本计划的提交区间内这两个路径为空。
  </action>
  <verify>
    <automated>pnpm test:ci</automated>
    <fails_when>`pnpm test:ci` 出现任何失败；或跳过数不等于 1（新增了 `it.skip`，或 D-06 的码 147 用例被取消 skip）；或文件数与「本计划不新增文件」不符；或 `vue-tsc --noEmit` 输出含本计划 6 个文件的任一路径；或某个提交的暂存内容含本计划**声明路径**（6 个源/测试文件 + `07-15-SUMMARY.md`）之外的文件（含工作树中既有的无关改动）；或本计划提交区间内 `.planning/ROADMAP.md`、`.planning/STATE.md` 出现改动（跟踪同步属 orchestrator 收尾步骤）；或 6 个文件的 diff 中出现被修改/删除的既有 jsDoc/注释行 —— 按 D-09 退出并修订计划，不得通过删改用例、放宽断言或撤回已提交改动来对齐门禁</fails_when>
  </verify>
  <acceptance_criteria>
    ...
    - Task 1..4 各有独立原子提交（`fix(07-15): …`），暂存内容只含本计划声明的文件；`git diff BASE_SHA..HEAD --stat` 不含本计划声明路径（6 个源/测试文件 + `07-15-SUMMARY.md`）之外的文件，且 `.planning/ROADMAP.md`/`.planning/STATE.md` 在本计划提交区间内为空
    ...
    - SUMMARY 含 Phase 7 验证失效与重跑要求，并明写 `.planning/ROADMAP.md`/`.planning/STATE.md` 的跟踪同步为本计划之外的 orchestrator 收尾步骤（本计划对这两个路径零编辑、零暂存、零提交；既有计划行与说明段不得被重排/删除/改写）
    - 未新建源码/测试文件、未新增依赖、未新增告警码、本计划提交区间内 `logger.json` 无改动（按区间判定，不看工作树）
  </acceptance_criteria>
  <done>D-44 三步门禁全绿、基线增量与预期一致、四条缺陷按缺陷原子提交且未牵连任何无关改动、评审映射与 Phase 7 验证失效说明写入 `07-15-SUMMARY.md`；`.planning/ROADMAP.md`/`.planning/STATE.md` 的跟踪同步作为 orchestrator 的收尾步骤在 SUMMARY 中明写，本计划零触碰</done>
</task>
...
5. 只改动 `files_modified` 的 6 个文件与本计划产出物 `07-15-SUMMARY.md`（两者合称本计划声明路径）；未新建源码/测试文件、未新增依赖、未改 `package.json`/`pnpm-lock.yaml`、未改 `hero/types.ts`/`replay/types.ts`/`replay/system.ts`/`flag/**`/`combat/mapDamage.ts`、未新增或修改告警码、本计划提交区间内 `logger.json` 无改动（按区间判定，不看工作树）；`.planning/ROADMAP.md`/`.planning/STATE.md` 在本计划提交区间内为空（跟踪同步为 orchestrator 收尾步骤）；工作树中与用户无关或属于用户的既有改动（未暂存修改/删除/未跟踪文件）未被触碰、未被暂存、未被提交。
...
8. `07-15-SUMMARY.md` 含四条评审映射与闭合状态、`WINDOWS.md` 未新建/未 `fixed`/未 `waive` 的明写、Phase 7 验证失效与重跑要求，以及「`.planning/ROADMAP.md`/`.planning/STATE.md` 的跟踪同步为 orchestrator 收尾步骤、本计划零触碰」的明写；本计划提交区间内这两个路径为空。
</verification>
...
- Phase 7 验证失效与重跑要求已记录（须重跑 `/gsd-verify-work`），评审映射登记在 `07-15-SUMMARY.md`；`.planning/ROADMAP.md`/`.planning/STATE.md` 的跟踪同步交由 orchestrator 收尾（本计划零编辑、零暂存、零提交）。
</success_criteria>
