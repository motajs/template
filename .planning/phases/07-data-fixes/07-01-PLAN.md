...
执行序：本计划在 Phase 7 中第一个执行。D-09 要求每个计划开始前暂停汇报修复方案并获用户确认（本计划 Task 0），因此不得与其他 Phase 7 计划并发执行（`depends_on` 为空仅表示源码层面无前置）。D-02：按系统切多个 PLAN（combat / enemy / replay / hero / map / flag+common / save / path）且系统内先高后低，本计划即其中的 combat 系统。
</objective>
