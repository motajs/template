...
- **引入位置**：只要模块引入了外部模块，那么此模块的第一行必须是 `import`，不得是注释。所有 `import` 中间不得加入任何注释，也不得加入任何空行，注释只允许出现在所有 `import` 之后。
...
- 测试文件必须在文件的每个 it 调用前使用单行注释说明当前覆盖的测试内容，测试范围变化时同步更新，便于后续人工审查。
...
- 所有的类必须先进行 `interface` 声明，之后由类 `implements` 之实现，不得出现没有 `implements` 的类。
- 通常情况下，需要向外暴露的接口类型都放到 `types.ts` 中，不要在其他文件中放向外暴露的接口类型。
